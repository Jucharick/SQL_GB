...
Для объединения выборок из двух таблиц нужно использовать
оператор UNION. При этом важно помнить, что:
-каждая выборка SELECT в рамках UNION должна иметь
...
-- 1. INNER JOIN: возвращает записи с совпадающими значениями в обеих таблицах.
SELECT messages.from_user_id, users.firstname, users.lastname, users.email, messages.to_user_id, messages.body, messages.created_at
FROM users
